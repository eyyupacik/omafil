...
public partial class GipePage : ContentPage
{
	public GipePage()
	{
		InitializeComponent();
    ...
    private double _turSayısı;

    public double TurSayısı
    {
        get { return _turSayısı; }
        set { _turSayısı = value; OnPropertyChanged(); }
    }
    ...
    private void TapGestureRecognizer_Tapped(object sender, EventArgs e)
    {
        ...
        TurSayısı = Convert.ToDouble(ACModel.tur_sayisi);
        ...
        if (IplikDenyesi == 0 && LikraDenyesi == 0 && Cekim == 0 && MakineHiz == 0 && TurSayısı == 0 && GozMiktar == 0 && BirimIpFiyat == 0 && BirimLikraFiyat == 0 && GMakineMaliyet == 0)
        {
            return;
        }
        else
        {
            GunlukUretim = (((MakineHiz / TurSayısı) * 60 * 24) / (9000 * 1000) * GozMiktar) * ((LikraDenyesi / Cekim) + IplikDenyesi);
            KullanilanIplik = ((((MakineHiz / TurSayısı) * 60 * 24) / (9000 * 1000) * GozMiktar) * ((LikraDenyesi / Cekim) + IplikDenyesi)) * (100 - ((LikraDenyesi / Cekim) / (LikraDenyesi / Cekim + IplikDenyesi)) * 100) / 100;
            KullanilanLikra = GunlukUretim - KullanilanIplik;
            Toplam = (((((MakineHiz / TurSayısı) * 60 * 24) / (9000 * 1000) * GozMiktar) * ((LikraDenyesi / Cekim) + IplikDenyesi)) * (100 - ((LikraDenyesi / Cekim) / (LikraDenyesi / Cekim + IplikDenyesi)) * 100) / 100) * BirimIpFiyat + ((GunlukUretim - KullanilanIplik) * BirimLikraFiyat) + GMakineMaliyet;
